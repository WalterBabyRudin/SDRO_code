...
#######################################################
################# Data Loading ########################
data = sklearn.datasets.load_svmlight_file("data/mg_scale.txt")
...
#                 maxiter = 1000, step_size0=1e-1, test_iterations=500)

# # Report Statistics
# SDRO_optval, residual = SDRO_optimizer.SDRO_eval(theta_SDRO_opt, SDRO_Lambda, SDRO_Reg, X_all, y_all)
SDRO_optval = 0.10024837244612644
residual = 0.17966725949092918
...
if input_method == "SG":
    theta_SG, theta_SG_hist, cost_SG_hist = SDRO_optimizer.SDRO_SG_solver(Dataloader_all, theta0, SDRO_Reg, SDRO_Lambda, 
                        maxiter = 100, learning_rate = 5e-3, silence=False, K_sample_max=4, test_iterations = 10)
    ...
    np.save("results/obj_SG_hist_mg.npy", obj_SG_hist)
    np.save("results/residual_SG_hist_mg.npy", residual_SG_hist)
    np.save("results/cost_SG_hist_mg.npy", cost_SG_hist)
...
if input_method == "RTMLMC":
    ...
    np.save("results/obj_RTMLMC_hist_mg.npy", obj_RTMLMC_hist)
    np.save("results/residual_RTMLMC_hist_mg.npy", residual_RTMLMC_hist)
    np.save("results/cost_RTMLMC_hist_mg.npy", cost_RTMLMC_hist)
...
if input_method == "MLMC":
    ...
    np.save("results/obj_MLMC_hist_mg.npy", obj_MLMC_hist)
    np.save("results/residual_MLMC_hist_mg.npy", residual_MLMC_hist)
    np.save("results/cost_MLMC_hist_mg.npy", cost_MLMC_hist)
...
if input_method == "RRMLMC":
    ...
    np.save("results/obj_RRMLMC_hist_mg.npy", obj_RRMLMC_hist)
    np.save("results/residual_RRMLMC_hist_mg.npy", residual_RRMLMC_hist)
    np.save("results/cost_RRMLMC_hist_mg.npy", cost_RRMLMC_hist)
...
if input_method == "RUMLMC":
    ...
    np.save("results/obj_RUMLMC_hist_mg.npy", obj_RUMLMC_hist)
    np.save("results/residual_RUMLMC_hist_mg.npy", residual_RUMLMC_hist)
    np.save("results/cost_RUMLMC_hist_mg.npy", cost_RUMLMC_hist)
